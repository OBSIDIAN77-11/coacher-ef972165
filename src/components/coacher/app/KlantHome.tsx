...
function SectionHead({ title, children }: { title: string; children?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between mt-6 mb-3">
      <h2 style={{ fontSize: 16, fontWeight: 800, color: "#FFFFFF" }}>{title}</h2>
      {children}
    </div>
  );
}
    ...
    <div style={{ padding: 18, borderRadius: 20, background: "#000000", border: "1px solid #1E2A44" }}>
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <div style={{ fontSize: 11, fontWeight: 700, color: "#8B98B0" }}>Herstel</div>
          <div style={{ fontSize: 42, fontWeight: 900, color: "#2563EB", letterSpacing: "-1px", lineHeight: 1, marginTop: 4 }}>
            {pct}%
          </div>
          <div style={{ fontSize: 12, color: "#FFFFFF", fontWeight: 700, marginTop: 6 }}>Klaar voor training</div>
        </div>
        <div className="relative flex items-center justify-center flex-shrink-0" style={{ width: 96, height: 96 }}>
          <svg width={96} height={96} style={{ transform: "rotate(-90deg)" }}>
            <circle cx={48} cy={48} r={r} stroke="#000000" strokeWidth={8} fill="none" />
            <circle
              cx={48}
              cy={48}
              r={r}
          ...
          <div className="absolute text-center">
            <div style={{ fontSize: 9, fontWeight: 800, color: "#8B98B0", letterSpacing: 0.5 }}>HRV</div>
            <div style={{ fontSize: 18, fontWeight: 900, color: "#FFFFFF", lineHeight: 1 }}>52</div>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-4 gap-2 mt-4">
        {bio.map((b) => (
          <div key={b.label} style={{ padding: 8, borderRadius: 10, background: "#000000", textAlign: "center" }}>
            <div style={{ fontSize: 13, fontWeight: 900, color: b.color, letterSpacing: "-0.3px" }}>{b.value}</div>
            <div style={{ fontSize: 9, color: "#8B98B0", fontWeight: 700, marginTop: 2 }}>{b.label}</div>
          </div>
        ))}
      </div>
    </div>
...
function SleepCard() {
  ...
  return (
    <div className="mt-3" style={{ padding: 18, borderRadius: 20, background: "#000000", border: "1px solid #1E2A44" }}>
      <div className="flex items-center justify-between">
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#8B98B0" }}>Slaapscore</div>
          <div style={{ fontSize: 28, fontWeight: 900, color: "#FFFFFF", marginTop: 2, letterSpacing: "-0.5px" }}>
            84<span style={{ fontSize: 14, color: "#8B98B0", fontWeight: 700 }}>/100</span>
          </div>
        </div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#8B98B0", textAlign: "right" }}>
          23:14 → 06:26
          <div style={{ fontSize: 13, color: "#FFFFFF", fontWeight: 800, marginTop: 2 }}>7.2u</div>
        </div>
      </div>
      ...
      <div className="flex justify-between mt-2">
        {phases.map((p) => (
          <div key={p.name} className="flex items-center gap-1">
            ...
            <span style={{ fontSize: 10, color: "#8B98B0", fontWeight: 700 }}>
              {p.name} {p.pct}%
            </span>
          </div>
        ))}
...
function Stat({ value, label, color, icon }: { value: string; label: string; color: string; icon?: React.ReactNode }) {
  return (
    <div style={{ padding: 12, borderRadius: 16, background: "#000000", border: "1px solid #1E2A44" }}>
      <div className="flex items-center gap-1" style={{ color }}>
        {icon}
        <span style={{ fontSize: 20, fontWeight: 900, letterSpacing: "-0.5px" }}>{value}</span>
      </div>
      <div style={{ fontSize: 11, color: "#8B98B0", fontWeight: 700, marginTop: 2 }}>{label}</div>
    </div>
  );
}
...
function TrainingCard() {
  ...
  return (
    <div className="mt-6" style={{ padding: 18, borderRadius: 20, background: "#000000", border: "1px solid #1E2A44" }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 800, color: "#FFFFFF" }}>Vrijdag: Billen & Benen</div>
            ...
            >
              <span
                ...
                style={{
                  ...
                  border: isDone ? "none" : "2px solid #4A5A75",
                }}
              >
              ...
              <div className="flex-1">
                <div
                  style={{
                    fontSize: 13,
                    fontWeight: 800,
                    color: isDone ? "#8B98B0" : "#FFFFFF",
                    textDecoration: isDone ? "line-through" : "none",
                  }}
                >
                  {e.name}
                </div>
                <div style={{ fontSize: 11, color: "#8B98B0", fontWeight: 600, marginTop: 1 }}>{e.spec}</div>
              </div>
            </button>
          );
        })}
